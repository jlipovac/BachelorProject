...
parser = argparse.ArgumentParser(
    prog='Assignment 3',
    description='Creates and saves two fasta files: A.fasta which contains all reads from the input reads file that '
                'start in the region 500,000-600,000, and B.fasta with the reads that start in the '
                'region 1,500,000-1,600,000. The output files are saved in the directory of this python script. The '
                'reads in the new files have IDs 1,2,3,...n. Finally, outputs the coverages of the two reads sets. '
                'Therefore, it is requested to present the reference sequence.')
...
for record in SeqIO.parse(args.reads, "fasta"):
    id, start = record.description.split("start=")[0].split(" ")[0], \
                record.description.split("start=")[1].split(", end=")[0]
    if (int(start) >= A_from and int(start) <= A_to):
        ...
        new_record = SeqRecord(record.seq, str(A_index), record.name, new_description, record.dbxrefs,
        ...
        new_record = SeqRecord(record.seq, str(B_index), record.name, new_description, record.dbxrefs,
                               record.features, record.annotations, record.letter_annotations)
